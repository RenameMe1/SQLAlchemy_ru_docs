...
SQL выражения обычно сопровождаются данными, которые будут переданы с сами выражением, как мы увидели в предыдущем INSERT примере. Следовательно метод [[Connection.execute()]] так же принимает параметры, которые известны как [[bound parameters]]. 